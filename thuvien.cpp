...
void dangnhap() {
	...
	//dem so dong de xin o nho cho con tro
	fstream f;
	string data;
	char ignore_char;
	int check, num_of_users = 0, num_of_accounts = 0;
	f.open("users_infor.txt", ios::in);
	do {
		num_of_accounts++;//dem so tai khoan
		getline(f, data); stringstream scin1(data);
		scin1 >> check;
		if (!scin1.fail())	num_of_users++;//dem so users
	} while (!f.eof());
	f.clear();
	f.close();
	//doc file
	...
	user* list_users = new user[num_of_users];
	for (int i = 0;i<num_of_users;i++) {
		scin >> list_users[i].sothutu;
		scin >> list_users[i].hovaten;
		scin >> list_users[i].ngaysinh.ngay;
		scin >> ignore_char;
		scin >> list_users[i].ngaysinh.thang;
		scin >> ignore_char;
		scin >> list_users[i].ngaysinh.nam;
		scin >> list_users[i].sodienthoai;
		scin >> list_users[i].email;
		scin >> list_users[i].giotinh;
		scin >> list_users[i].vaitro;
		scin >> list_users[i].sotaikhoan;
		for (int j = 0;j < list_users[i].sotaikhoan;j++) {
			scin >> list_users[i].taikhoan_matkhau[j].Tendangnhap;
			scin >> list_users[i].taikhoan_matkhau[j].matkhau;
		}
	}
	f.close();//dong file
	...
	while (thoat) {
		...
		if ((cv == 3) && (c == 13)) {
			int dem = 0;
			boolean bOutOfLoop = false;
			for (int i = 0;i < num_of_users&&!bOutOfLoop;i++) {
				for (int j = 0;j < list_users[i].sotaikhoan && !bOutOfLoop;j++) {
					if ((TDN == list_users[i].taikhoan_matkhau[j].Tendangnhap) && (MK == list_users[i].taikhoan_matkhau[j].matkhau)) {
						gotoxy(0, 40);
	...
	delete[] list_users;list_users = NULL;
}
...
void dangki() {
	user A;//khoi tao 1 user moi
	string xnmk = "";//bien luu mat khau xac nhan xem co trung voi mat khau da nhap hay ko
	int x1 = 15, x2 = 15, x3 = 15, x4 = 15, x5 = 22, x6 = 37, x7 = 51, x8 = 15, x9 = 15;//hoanh do nhap chuoi thong tin cua cac du lieu
	indangki();
	...
	while (thoat) {
		...
		if ((cv == 2) && (c == 13)) { nhapngay(x5, x6, x7, A.ngaysinh.ngay, A.ngaysinh.thang, A.ngaysinh.nam); }//nhap ngay thang nam sinh
		...
		if ((cv == 6) && (c == 13)) { xulichuoi(A.taikhoan_matkhau[0].Tendangnhap, 'y', x1, 36); }//nhap ten dang nhap
		if ((cv == 7) && (c == 13)) { xulichuoi(A.taikhoan_matkhau[0].matkhau, 'n', x2, 43); }//nhap mat khau
		...
		if ((cv == 9) && (c == 13)) {//in ra file luu nguoi dung
			...
			//cach ghi them vao file ma khong xoa file cu

			/*string data;
			int num_of_users = 0, check;
			fstream f;
			f.open("users_infor.txt", ios::in);
			do {
				getline(f, data); stringstream scin1(data);
			...
			ostringstream connect_string;
			connect_string << num_of_users + 1 << '\t' << A.hovaten << '\t' << A.sodienthoai << '\t' << A.email << '\t' << A.giotinh << '\t' << "doc_gia" << '\t' << 1 << '\t' << A.taikhoan_matkhau[0].Tendangnhap << '\t' << A.taikhoan_matkhau[0].matkhau << " ";
			string add = connect_string.str();

			data += add;
			f.close();
			f.open("users_infor.txt", ios::out);
			f << data;
			f.close();
			// 4. Đóng file*/
		}
