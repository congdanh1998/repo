...
void dangki(){
	...
	A.vaitro="doc_gia";A.sotaikhoan=1;A.sothutu=12;/*so thu tu tiep theo gia su la 12*/
	...
	while(thoat){
		...
		if ((cv==4) && (c==13)){xulichuoi(A.MSSV,'y',x8,29,19);}//nhap so dien thoai
		...
		if ((cv==9) && (c==13)){//in ra file luu nguoi dung
			ofstream outfile;
			outfile.open("users_infor.txt",ios::app);//ghi vo file lun
			//outfile.write(reinterpret_cast<char*>(&A), sizeof(user));//ma hoa thanh nhi phan ma ko bit co tac dung ko
			outfile<<endl<<A.sothutu<<"   "<<A.hovaten<<"   "
				...
				<<A.vaitro<<"   "
				<<A.sotaikhoan<<"    "
				<<A.taikhoan_matkhau[0].Tendangnhap<<"   "
				<<A.taikhoan_matkhau[0].matkhau<<"  ";
				outfile.close();
			thoat=0;
		}
